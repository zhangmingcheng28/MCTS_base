...
class AirspaceState():
    ...
    def reward(self, alpha=0.7, R_A=30):
        """
...
if __name__ == "__main__":
    ...
    for ea_eps in range(args.episodes):
        ...
        host_uav_start = [259.47, 145.66, 35]  # for two intru case, host start use [258, 148, 35.25]
        host_uav_end = [352.02, 120.93, 35]

        host_uav_end = extend_endpoint(host_uav_start, host_uav_end, extension=60)
        ...
        # intruder2_uav_end = [212.0, 138.36, 30]
        ...
        # Example usage: Suppose you have 5 intruders. We'll pick a fraction for each
        host_start_example = [259.47, 145.66, 35]
        host_end_example = [352.02, 120.93, 35]  # or wherever your host ends
        intruders = [
            (intruder1_uav_start, intruder1_uav_end),  # intruder1
            (intruder2_uav_start, intruder2_uav_end),  # intruder2
            (intruder3_uav_start, intruder3_uav_end),  # intruder3
            (intruder4_uav_start, intruder4_uav_end),  # intruder4
            (intruder5_uav_start, intruder5_uav_end)  # intruder5
        ]

        clamped_intruders = clamp_intruder_start_altitude(intruders, z_max=70.0)
        fractions = [0.4, 0.4, 0.7, 0.7, 0.9]  # pick different conflict times for each intruder

        new_intruders = []
        for i, (start_i, end_i) in enumerate(clamped_intruders):
            frac = fractions[i]
            new_end = redesign_intruder_end_for_conflict(
                host_start_example, host_end_example,
                intruder_start=start_i,
                speed_host=10.0, speed_intruder=15.0,
                fraction=frac
            )
            # extend end point
            new_end = extend_endpoint(start_i, new_end, extension=18)
            new_intruders.append((start_i, new_end))
            print(f"Intruder {i + 1} new end: {new_end}")


        # # Compute extended endpoints:
        # intruder1_uav_end = extend_endpoint(intruder1_uav_start, intruder1_uav_end, extension=15)
        # intruder2_uav_end = extend_endpoint(intruder2_uav_start, intruder2_uav_end, extension=15)
        # intruder3_uav_end = extend_endpoint(intruder3_uav_start, intruder3_uav_end, extension=15)
        # intruder4_uav_end = extend_endpoint(intruder4_uav_start, intruder4_uav_end, extension=15)
        # intruder5_uav_end = extend_endpoint(intruder5_uav_start, intruder5_uav_end, extension=15)
        #
        # intruder1_uav_start = shorten_line(intruder1_uav_end, intruder1_uav_start, 100)
        # intruder2_uav_start = shorten_line(intruder2_uav_end, intruder2_uav_start, 100)
        # intruder3_uav_start = shorten_line(intruder3_uav_end, intruder3_uav_start, 25)
        # intruder4_uav_start = shorten_line(intruder4_uav_end, intruder4_uav_start, 25)
        # intruder5_uav_start = shorten_line(intruder5_uav_end, intruder5_uav_start, 80)


        intruder_norminal_trajectories1 = interpolate_trajectory(new_intruders[0][0], new_intruders[0][1], speed=15)
        intruder_norminal_trajectories2 = interpolate_trajectory(new_intruders[1][0], new_intruders[1][1], speed=15)
        intruder_norminal_trajectories3 = interpolate_trajectory(new_intruders[2][0], new_intruders[2][1], speed=15)
        intruder_norminal_trajectories4 = interpolate_trajectory(new_intruders[3][0], new_intruders[3][1], speed=15)
        intruder_norminal_trajectories5 = interpolate_trajectory(new_intruders[4][0], new_intruders[4][1], speed=15)

        intruder_norminal_trajectories = [intruder_norminal_trajectories1, intruder_norminal_trajectories2,
                                          intruder_norminal_trajectories3, intruder_norminal_trajectories4,
                                          intruder_norminal_trajectories5]
        ...
        with open('five_intru_traj.pickle', 'rb') as handle:
            b = pickle.load(handle)
        host_trajectory = b[0]
        intruder_positions_history = b[1]
        episode_relative_dist_record = b[2]
        plot_one_intru_head_on(building_polygons, host_trajectory, intruder_positions_history,
                               x_limit, y_limit, host_uav_simulation_start, host_uav_simulation_end, 3)
        plot_relative_distances(episode_relative_dist_record, num_intruders, delta_t=3)
    ...
    output_filename = f"simulation_results_episodes_{args.episodes}_delta_{delta_t}.pickle"
